...
module.exports = {
  ...
  assignBasicSnippet: assignBasicSnippet,
...
async function getAllUsers (testMode = false) {
  ...
  return sqlGet(sqlCode, [], testMode)
}
...
async function deleteSnippet (index, req, res, testMode = false) {
  ...
  return sqlPut(sqlCode, index, testMode)
}
...
async function forwardSnippet (snippetid, req, res, testMode = false) {
  ...
  return deleteSnippet(snippetid, req, res)
}
...
async function sendSnippetToRandomUsers (data, res, req, testMode = false) {
  ...
  return createTwoSnippetsViaContent(snippetContent, fromRedirect, toRedirect0, toRedirect1, testMode).then(res => {
    return res
  })
}

async function assignBasicSnippet (userid, testMode = false) {
  let sqlCode = 'SELECT * FROM login WHERE id = ?'

  let redirectid = await sqlGet(sqlCode, userid)
  console.log(redirectid)
  return sendSnippetToUser(0, redirectid[0].redirectid, testMode)
}
...
async function createSnippetContent (description, url, sender, testMode = false) {
  ...
  return sqlPut(sqlCode, sqlData, testMode).then(res => {
    return res
  })
}
...
async function deleteSnippetContent (contentid, testMode = false) {
  ...
  await sqlPut(sqlCode, contentid, testMode).then(res => {
    return res
  })
  sqlCode = 'DELETE FROM snippet WHERE contentid = ?'
  return sqlPut(sqlCode, contentid, testMode)
}
